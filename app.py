...
def input(email):
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT * FROM dataset2")
...
def search(email):
    ...
    if request.method == "POST":    
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        recipecat="SELECT * FROM dataset2"
        cursor.execute(recipecat)
        mysql.connection.commit()
        recipecat = cursor.fetchall()
        dataset2=pd.DataFrame(recipecat)
        recipelist=findbyingredient(RecipeCategory,dataset2)
        ...
        if len(recipelist)>0 and len(dish)==0:
            ...
            cursor.execute('SELECT * from dataset2 WHERE RecipeId IN' +where_in,recipelist)
            #selector='SELECT * from dataset WHERE Name LIKE "%%'+dish+'%%"'+'AND RecipeCategory IN' +where_in,ingredients
            #cursor.execute(selector)

        else:
            cursor = mysql.connection.cursor()
            selector='SELECT * from dataset2 WHERE Name LIKE "%'+dish+'%" OR RecipeCategory LIKE "%%'+dish+'%%"'
            cursor.execute(selector)
        mysql.connection.commit()
...
def profile(email):  
        ...
        email=account["email"]
        cursor = mysql.connection.cursor()
        cursor.execute("select * from dataset2 where RecipeId in (SELECT dish_id from ratingdb where id=%s)",[id])
        mysql.connection.commit()
        data = cursor.fetchall()
        if request.method == "POST": 
            dish = request.form['dish']  
            cursor.execute('SELECT * from dataset2 WHERE  RecipeId in (SELECT dish_id from ratingdb where id=%s) and Name LIKE "%%'+dish+'%%"',[id])
            mysql.connection.commit()
            data = cursor.fetchall()
